...
namespace CS_Parallel_Processing
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Using Sequential Processing wuth for.oop");
            var startTimerSeq = Stopwatch.StartNew();

            for (int i = 0; i < employees.Count; i++)
            {
                employees[i] = ProcessTax.CalculateTax(employees[i]);
                Console.WriteLine($"In Sequenatail Execution Tax for EmpName {employees[i].EmpName} is = {employees[i].Tax}");
            }
            Console.WriteLine();
            Console.WriteLine($"Total Time for Sequential Executions = {startTimerSeq.Elapsed.TotalSeconds}");
            ...
            Console.WriteLine();
            Console.WriteLine($"Total Time for Parallel Executions = {startTimerParallel.Elapsed.TotalSeconds}");
